...
await sharp('public/images/logo-omt.png').resize({ width: 240 }).webp({ quality: 75, alphaQuality: 70, effort: 6 }).toFile('public/images/logo-omt-header.webp')

// Images de partage (Facebook, WhatsApp…) : photo recadrée + bandeau orange
mkdirSync('public/images/og', { recursive: true })
const og = {
  'og-maconnerie.jpg': 'terrasse-entree-dallage-travertin.webp',
...
const logo = await sharp('public/images/logo-omt-dark.webp').resize({ width: 230 }).toBuffer()
const bar = Buffer.from('<svg width="1200" height="12"><rect width="1200" height="12" fill="#e67e22"/></svg>')
for (const [name, src] of Object.entries(og)) {
  await sharp(dir + src).resize(1200, 630, { fit: 'cover', position: 'attention' })
    .composite([{ input: logo, left: 940, top: 458 }, { input: bar, left: 0, top: 618 }])
